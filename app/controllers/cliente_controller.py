...
class Cliente_Controller:
    ...
    def save(self):
        try:
            ...
            self.get_all()
            self.view.exibir_mensagem((Idioma.t("cliente.cadastrado")))
    ...
    def update(self):
        try:
            if self.cliente_selecionado is None:
                self.view.exibir_mensagem((Idioma.t("cliente.selecione_na_lista", False)))
                return
            nome, data_nascimento, limite_credito, cidade = self.view.ler_dados_cliente()
            ...
            self.get_all()
            self.view.exibir_mensagem((Idioma.t("cliente.atualizado")))
        except ValueError as e:
            self.view.exibir_mensagem(f"Erro: {str(e)}", False)
    ...
    def delete(self):
        ...
        try:
            ...
            if sucesso:
                ...
                self.view.limpar_campos()
                self.get_all()
                self.view.exibir_mensagem((Idioma.t("cliente.excluido")))
            else:
                self.view.exibir_mensagem((Idioma.t("cliente.nao_encontrado", False)))
        except Exception as e:
            self.view.exibir_mensagem((Idioma.t("cliente.problema_excluir", False)))
